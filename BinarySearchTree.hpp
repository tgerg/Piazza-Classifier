...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    return (empty_impl(node->left) || (less(max_element_impl(node->left)->datum, node->datum) && check_sorting_invariant_impl(node->left, less)))
         && (empty_impl(node->right) || (less(node->datum, min_element_impl(node->right)->datum) && check_sorting_invariant_impl(node->right, less)));
    return false;
  }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if (!node) {
      ...
      Node * min = min_greater_than_impl(node->left, val, less);
      if (!min) {
        return node;
      }else {
        return min;
